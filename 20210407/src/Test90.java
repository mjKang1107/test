...
class Employee{
	...
	//사원의 이름과 급여를 하나의 문자열로 만들어 반활 getEmployee메소드

	public String getEmployee() {
		return  name + ", " + salary ;
	}
	
	//기본생성자 -> 아무런일도 하지않음, 비워두자
	public Employee() {
...
class Manager extends Employee{
	...
	//name과 salary 그리고depart변수의 값을 초기화 하는 생성자
	//->Manager객체 내부에 있는 name인스턴스변수와 salary인스턴스는 상속받아 사용하므로
	//Employee 객체의 생성자를 다시 호출해 (super(name,salary)) 넘겨주어
	//
...
public class Test90 {
	...
	public static void main(String[] args) {
		// Employee객체 생성시 생성자로 "홍길동", 2000을 전달하여 변수값 초기화(참조변수 이름 emp)
		Employee emp = new Employee("홍길동", 2000);
		// Manager객체 생성시 생성자호출! "이순신",4000,"개발"을 전달하여 변수값 초기화(이름 man)
