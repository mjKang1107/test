...
public class Test116 {
	...
	public static void main(String[] args) {
		...
		if(a == b) {
			System.out.println("두 객체 메모리는 같다");
		...
		// -> 두객체가 동일한 메모리에 할당 되어 잇는지 물어보기 위해 사용
		...
		//두 객체 메모리가 같은 객체메모리를 말하는지 == 연산자를 이용해서 비교 해보자
		PointTest1 c = new PointTest1(2, 3);
		PointTest1 d = new PointTest1(2, 3);
		PointTest1 e;
		e = c;
		//c참조변수에 저장된 객체메모리의 주소와 d참조변수에 저장된 객체메모리의 주소가 같은지 비교	
		if(c == d) {
			System.out.println("c == d");
		}
		if(c == e) {
			System.out.println("c == e");//c == e 출력됨
		}
		//결론~~~ 
		//1. 기본자료형(기본데이터)비교시 == 연산자 사용하면
		//   데이터가 같은지 비교 한다.
		//2. 참조자료형(객체)비교시 == 연산자 사용하면
		//   두 참조변수가 동일한 객체 메모리를 가리켜서 참조하고 있는지를 비교한다.
		//   두객체 메모리의 주소를 비교한다.
