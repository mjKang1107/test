...
class FriutSeller{//과일 판매자 
	...
	//메소드
	//과일 판매 하는 행동을 메소드로 선언
	int saleApple(int money) {//과일 구매자가 과일구매시 전달 하는 금액을 매개변수로 받음
		//판매한 과일 개수 구하여 저장
		//단! 과일 1개당 1000원이라고 가정
		int num = money / 1000;
		
		//과일 판매후 과일 개수 차감 ->  numOfApple변수에 저장
		numOfApple -= num;
...
//[과일 구매자 관점]
//1단계  나(과일구매자) 모델링
//      데이터 ->  현재 소유하고 있는 10000원, 현재 소유한 과일수 100개
//		동작(기능) -> 과일구매하기, 과일구매후 현재 상황을 말할수 있다. 
...
class FruitBuyer{ //과일 구매자 클래스 설계 
	...
	//메소드
	//과일구매하기 행동의 메소드			new FriutSeller();   2000
	//매개변수선언 -> 과일구매하기 위한 과일판매자 객체를 전달 하기 위한 매개변수 선언
	...
	public void buyApple(FriutSeller seller, int money) {
		...
		//new FruitSeller()객체의 과일판매행동의 메소드 호출시 금액을 전달함.
		//전달후 구매한 과일 개수를 리턴 받아 -> numOffApple변수에 누적
		numOfApple += seller.saleApple(money);
		
		//과일 판매자에게 과일 구매후 현재 과일 구매자가 소유하고 있는 현금 계산
		myMoney -= money;	
	}
	
	//과일 구매후 과일구매자의 현재상황 출력기능의 메소드 
...
//3단계  위 2개의 클래스를 이용하여 객체 생성 후 사용
	...
	FriutSeller seller = new FriutSeller();
